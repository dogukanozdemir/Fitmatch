...
class UserControllerTest {
    ...
    void getUser_notFound() throws Exception {
        ...
        when(userService.getUserById(eq(id)))
                .thenThrow(new ResponseStatusException(HttpStatus.NOT_FOUND, "user with id does not exist"));
    ...
    void completeProfile_ok() throws Exception {
        ...
        mockMvc.perform(post("/api/users/profile").contentType(APPLICATION_JSON).content(body))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(APPLICATION_JSON))
                .andExpect(jsonPath("$.email").value("x@y.com"))
    ...
    void completeProfile_unauthorized() throws Exception {
        CompleteProfileRequest req = new CompleteProfileRequest(
                FitnessLevel.BEGINNER, Set.of(Activity.RUNNING), 41.0, 29.0, 10);
